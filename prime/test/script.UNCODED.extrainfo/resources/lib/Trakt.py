...
def handle_trakt_movies(results):
    ...
    for movie in results:
        ...
        movie = {'title': movie["movie"]["title"],
                 ...
                 'Trailer': 'plugin://script.UNCODED.extrainfo/?info=playtrailer&id=%s' % str(movie["movie"]["ids"]["tmdb"]),
                 'year': movie["movie"]["year"],
                 'id': id,
                 'imdb_id': movie["movie"]["ids"]["imdb"],
                 'trakt_id': movie["movie"]["ids"]["trakt"],
                 'slug': movie["movie"]["ids"]["slug"],
                 'path': 'plugin://script.UNCODED.extrainfo/?info=UNCODED.extrainfo&id=%s' % str(movie["movie"]["ids"]["tmdb"]), 
...
def handle_trakt_tvshows(results):
    ...
    for tvshow in results:
        ...
        show = {'title': tvshow['show']["title"],
                ...
                'Trailer': 'plugin://script.UNCODED.extrainfo/?info=playtvtrailer&id=%s' % str(tvshow['show']['ids']["tmdb"]),
                ...
                'path': 'plugin://script.UNCODED.extrainfo/?info=extendedtvinfo&id=%s' % str(tvshow['show']['ids']["tmdb"]),
